    ...
    try:
        ...
        with tqdm(total=total_records, desc="Progression", unit="req") as pbar:
            for offset in range(0, total_records, page_size):
                # Calcul du nombre exact de résultats à récupérer pour cette page
                remaining = total_records - len(all_results)
                ...
                # Limitation stricte au nombre total annoncé
                results_to_add = results[:remaining]
                all_results.extend(results_to_add)
                pbar.update(len(results_to_add))
